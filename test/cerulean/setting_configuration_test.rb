...
class Cerulean::SettingConfigurationTest < Minitest::Test
  ...
  def test_setting_requires_type
    assert_raises Cerulean::InvalidType do
      Cerulean::SettingConfiguration.new(@name)
    end
  end

  def test_setting_configuration_only_needs_name_and_type_and_sets_defaults
    setting = Cerulean::SettingConfiguration.new(@name, type: @type)
  ...
  def test_requires_valid_type
    assert_raises Cerulean::InvalidType do
      Cerulean::SettingConfiguration.new(@name, type: 'invalid_type')
    end
  ...
  def test_chain_on_must_be_valid
    assert_raises Cerulean::InvalidChainOption do
      Cerulean::SettingConfiguration.new(@name, type: @type, chain_on: :bad_chain_on)
    end

    Cerulean::SettingConfiguration::CHAINING_OPTIONS.each do |chain_on|
      setting = Cerulean::SettingConfiguration.new(@name, type: @type, chain_on: chain_on)
      assert_equal chain_on, setting.chain_on
    end
  end

  def test_sets_default_value
    setting = Cerulean::SettingConfiguration.new(@name, type: @type, default: @default)
    assert_equal @default, setting.default
  end

  def test_sets_validations_to_array
    validation = { presence: true }
    setting = Cerulean::SettingConfiguration.new(@name, type: @type, validations: validation)
