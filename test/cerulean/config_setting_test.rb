...
class Cerulean::ConfigSettingTest < Minitest::Test
  def setup
    @name     = 'some_name'
    @type     = Cerulean::ConfigSetting::VALID_TYPES.first
    @chain_on = Cerulean::ConfigSetting::CHAINING_OPTIONS.first
    @default  = 1
  end
  ...
  def test_requires_valid_type
    assert_raises Cerulean::InvalidType do
      Cerulean::ConfigSetting.new(@name, 'invalid_type')
    end

    Cerulean::ConfigSetting::VALID_TYPES.each do |type|
      setting = Cerulean::ConfigSetting.new(@name, type)
  ...
  def test_chain_on_must_be_valid
    assert_raises Cerulean::InvalidChainOption do
      Cerulean::ConfigSetting.new(@name, @type, chain_on: :bad_chain_on)
    end

    Cerulean::ConfigSetting::CHAINING_OPTIONS.each do |chain_on|
      setting = Cerulean::ConfigSetting.new(@name, @type, chain_on: chain_on)
      assert_equal chain_on, setting.chain_on
    end
  end

  def test_sets_default_value
    setting = Cerulean::ConfigSetting.new(@name, @type, default: @default)
    assert_equal @default, setting.default
  end

  def test_sets_validations_to_array
    validation = { presence: true }
    setting = Cerulean::ConfigSetting.new(@name, @type, validations: validation)
